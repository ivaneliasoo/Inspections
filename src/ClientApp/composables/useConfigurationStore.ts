...
export const useConfigurationStore = defineStore('configuration', {
  ...
  actions: {
    async getConfigurations (payload) {
      const configs = await window.$nuxt.$axios.$get(`reportconfiguration?${payload ?? ''}`)
    ...
    async createConfiguration (payload: AddReportConfigurationCommand | ReportConfiguration) {
    ...
    async updateConfiguration (payload: UpdateReportConfigurationCommand) {
      return await window.$nuxt.$axios.$put(`reportconfiguration/${payload.id}`, payload)
    },
    async deleteConfiguration (payload: number) {
      return await window.$nuxt.$axios.$delete(`reportconfiguration/${payload ?? 0}`)
        .then(() => {
