...
export const useSignaturesStore = defineStore('signatures', {
  ...
  actions: {
    async getSignatures(payload) {
      const sign = await window.$nuxt.$axios.$get('signatures', {
        params: payload,
      })
      this.setList(sign)
    },
    ...
    setList(value: SignatureDTO[]) {
      this.signaturesList = value
    },
    remove(value: number) {
      this.signaturesList = this.signaturesList?.filter((s) => s.id !== value)
    },
    add(value: SignatureDTO) {
      this.signaturesList?.splice(0, 0, value)
    },
