...
export const useReportsStore = defineStore('reports', {
  ...
  actions: {
    async getReports (payload) {
      const reports = await window.$nuxt.$axios.$get('reports', { params: payload })
    ...
    async getReportPhotos (payload) {
      const photos = await window.$nuxt.$axios.$get(`reports/${payload}/photorecord`)
      return photos
    },
    ...
    SET_REPORT_LIST (value: Report[]) { this.reportList = value },
    REMOVE_REPORT (value: number) { this.reportList = this.reportList.filter(r => r.id !== value) }
  }
})
