...
public class ReportsConfigurationsRepository : IReportConfigurationsRepository
{
    ...
    public ReportsConfigurationsRepository(InspectionsContext context,
        ILogger<ReportsConfigurationsRepository> logger)
    {
    ...
    public async Task<ReportConfiguration> GetByIdAsync(int id)
    {
        ...
        var signaturesDef = _context.Signatures.Where(s =>
            s.IsConfiguration && s.ReportId == null && s.ReportConfigurationId == id);
